...
namespace NzbDrone.Core.Test.Profiles.Delay
{
    ...
    public class DelayProfileServiceFixture : CoreTest<DelayProfileService>
    {
        ...
        [Test]
        public async Task should_move_to_first_if_afterId_is_null()
        {
            var moving = _last;
            var result = (await Subject.Reorder(moving.Id, null)).OrderBy(d => d.Order).ToList();
        ...
        [Test]
        public async Task should_move_after_if_afterId_is_not_null()
        {
            var after = _first;
            var moving = _last;
            var result = (await Subject.Reorder(moving.Id, _first.Id)).OrderBy(d => d.Order).ToList();
            var moved = result[1];
        ...
        [Test]
        public async Task should_reorder_delay_profiles_that_are_after_moved()
        {
            var moving = _last;
            var result = (await Subject.Reorder(moving.Id, null)).OrderBy(d => d.Order).ToList();
        ...
        [Test]
        public async Task should_not_change_afters_order_if_moving_was_after()
        {
            var after = _first;
        ...
        [Test]
        public async Task should_change_afters_order_if_moving_was_before()
        {
            var after = _last;
            var afterOrder = after.Order;
            var moving = _first;

            var result = (await Subject.Reorder(moving.Id, after.Id)).OrderBy(d => d.Order).ToList();
