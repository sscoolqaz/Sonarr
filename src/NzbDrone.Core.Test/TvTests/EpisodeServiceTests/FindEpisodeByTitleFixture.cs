...
namespace NzbDrone.Core.Test.TvTests.EpisodeServiceTests
{
    ...
    public class FindEpisodeByTitleFixture : CoreTest<EpisodeService>
    {
        ...
        [Test]
        public async Task should_find_episode_by_title()
        {
            const string expectedTitle = "A Journey to the Highlands";
            GivenEpisodesWithTitles(expectedTitle);

            (await Subject.FindEpisodeByTitle(1, 1, "Downton.Abbey.A.Journey.To.The.Highlands.720p.BluRay.x264-aAF"))
                   .Title
                   .Should()
                   .Be(expectedTitle);
        }
        ...
        [Test]
        public async Task should_prefer_longer_match()
        {
            ...
            (await Subject.FindEpisodeByTitle(1, 1, "The.Walking.Dead.S04.Special.Inside.The.Walking.Dead.Walker.University.720p.HDTV.x264-W4F"))
                   .Title
                   .Should()
                   .Be(expectedTitle);
        }
        ...
        [Test]
        public async Task should_return_null_when_no_match_is_found()
        {
        ...
        [Test]
        public async Task should_handle_e00_specials()
        {
            const string expectedTitle = "Inside The Walking Dead: Walker University";
            GivenEpisodesWithTitles("Inside The Walking Dead", expectedTitle, "Inside The Walking Dead Walker University 2");

            (await Subject.FindEpisodeByTitle(1, 1, "The.Walking.Dead.S04E00.Inside.The.Walking.Dead.Walker.University.720p.HDTV.x264-W4F"))
                   .Title
                   .Should()
                   .Be(expectedTitle);
        }
        ...
        public async Task should_handle_special(string releaseTitle, string expectedTitle, string[] rejectedTitles)
        {
            GivenEpisodesWithTitles(rejectedTitles.Concat(new[] { expectedTitle }).ToArray());

            var episode = await Subject.FindEpisodeByTitle(1, 0, releaseTitle);
        ...
        [Test]
        public async Task should_handle_special_with_apostrophe_in_title()
        {
