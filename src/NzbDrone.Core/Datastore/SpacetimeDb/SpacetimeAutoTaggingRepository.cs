...
namespace NzbDrone.Core.Datastore.SpacetimeDb
{
    public class SpacetimeAutoTaggingRepository : SpacetimeBasicRepository<AutoTag, StdbAutoTag>, IAutoTaggingRepository
    {
        ...
        protected override IDisposable SubscribeOwnDeleteCommitted(Action<int> onCommitted, Action<Exception> onFailed)
        {
            ...
            Conn.Connection.Reducers.OnDeleteAutoTag += Handler;
            return new Unsubscriber(() => Conn.Connection.Reducers.OnDeleteAutoTag -= Handler);
        }

        protected override IDisposable SubscribeOwnInsertCommitted(Action onCommitted, Action<Exception> onFailed)
        {
            void Handler(ReducerEventContext ctx, string p1, string p2, bool p3, string p4)
            {
                if (ctx.Event.CallerIdentity == Conn.Connection.Identity &&
                    ctx.Event.CallerConnectionId == Conn.Connection.ConnectionId &&
                    ctx.Event.Status is Status.Committed)
            ...
            Conn.Connection.Reducers.OnInsertAutoTag += Handler;
            return new Unsubscriber(() => Conn.Connection.Reducers.OnInsertAutoTag -= Handler);
        }
