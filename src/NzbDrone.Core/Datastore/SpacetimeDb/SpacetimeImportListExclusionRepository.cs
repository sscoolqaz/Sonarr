...
namespace NzbDrone.Core.Datastore.SpacetimeDb
{
    public class SpacetimeImportListExclusionRepository : SpacetimeBasicRepository<ImportListExclusion, StdbImportListExclusion>, IImportListExclusionRepository
    {
        ...
        protected override IDisposable SubscribeOwnDeleteCommitted(Action<int> onCommitted, Action<Exception> onFailed)
        {
            ...
            Conn.Connection.Reducers.OnDeleteImportListExclusion += Handler;
            return new Unsubscriber(() => Conn.Connection.Reducers.OnDeleteImportListExclusion -= Handler);
        }

        protected override IDisposable SubscribeOwnInsertCommitted(Action onCommitted, Action<Exception> onFailed)
        {
            void Handler(ReducerEventContext ctx, int p1, string p2)
            {
                if (ctx.Event.CallerIdentity == Conn.Connection.Identity &&
                    ctx.Event.CallerConnectionId == Conn.Connection.ConnectionId &&
                    ctx.Event.Status is Status.Committed)
            ...
            Conn.Connection.Reducers.OnInsertImportListExclusion += Handler;
            return new Unsubscriber(() => Conn.Connection.Reducers.OnInsertImportListExclusion -= Handler);
        }
