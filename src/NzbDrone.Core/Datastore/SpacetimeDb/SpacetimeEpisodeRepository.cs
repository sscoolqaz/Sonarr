...
namespace NzbDrone.Core.Datastore.SpacetimeDb
{
    ...
    public class SpacetimeEpisodeRepository : SpacetimeBasicRepository<Episode, StdbEpisode>, IEpisodeRepository
    {
        ...
        public override Task MigrateInsert(Episode model) => InvokeAndWaitForMigrateInsert(model.Id, () => Conn.Connection.Reducers.MigrateInsertEpisode(
            model.Id,
            model.SeriesId,
            model.TvdbId,
            model.EpisodeFileId,
        ...
        public async Task<Episode> Find(int seriesId, int season, int episodeNumber) =>
            (await All()).SingleOrDefault(e => e.SeriesId == seriesId && e.SeasonNumber == season && e.EpisodeNumber == episodeNumber);

        public async Task<Episode> Find(int seriesId, int absoluteEpisodeNumber) =>
            (await All()).SingleOrDefault(e => e.SeriesId == seriesId && e.AbsoluteEpisodeNumber == absoluteEpisodeNumber);

        public async Task<List<Episode>> Find(int seriesId, string date) =>
            (await All()).Where(e => e.SeriesId == seriesId && e.AirDate == date).ToList();

        public async Task<List<Episode>> GetEpisodes(int seriesId) => (await All()).Where(e => e.SeriesId == seriesId).ToList();

        public async Task<List<Episode>> GetEpisodes(int seriesId, int seasonNumber) =>
            (await All()).Where(e => e.SeriesId == seriesId && e.SeasonNumber == seasonNumber).ToList();

        public async Task<List<Episode>> GetEpisodesBySeriesIds(List<int> seriesIds) =>
            (await All()).Where(e => seriesIds.Contains(e.SeriesId)).ToList();

        public async Task<List<Episode>> GetEpisodesBySceneSeason(int seriesId, int sceneSeasonNumber) =>
            (await All()).Where(e => e.SeriesId == seriesId && e.SceneSeasonNumber == sceneSeasonNumber).ToList();

        public async Task<List<Episode>> GetEpisodeByFileId(int fileId) => (await All()).Where(e => e.EpisodeFileId == fileId).ToList();

        public async Task<List<Episode>> EpisodesWithFiles(int seriesId)
        {
            var files = (await _mediaFileRepository.GetFilesBySeries(seriesId)).ToDictionary(f => f.Id);
        ...
        public async Task<PagingSpec<Episode>> EpisodesWhereCutoffUnmet(PagingSpec<Episode> pagingSpec, List<QualitiesBelowCutoff> qualitiesBelowCutoff, bool includeSpecials, HashSet<int> seriesTags = null, List<int> quality = null)
        {
            var startingSeasonNumber = includeSpecials ? 0 : 1;
            var seriesById = (await _seriesRepository.All()).ToDictionary(s => s.Id);
            var filesById = (await _mediaFileRepository.All()).ToDictionary(f => f.Id);
            ...
            foreach (var episode in (await All()).Where(e => e.EpisodeFileId != 0 && e.SeasonNumber >= startingSeasonNumber))
            {
        ...
        public async Task<List<Episode>> FindEpisodesBySceneNumbering(int seriesId, int seasonNumber, int episodeNumber) =>
            (await All()).Where(e => e.SeriesId == seriesId && e.SceneSeasonNumber == seasonNumber && e.SceneEpisodeNumber == episodeNumber).ToList();

        public async Task<List<Episode>> FindEpisodesBySceneNumbering(int seriesId, int sceneAbsoluteEpisodeNumber) =>
            (await All()).Where(e => e.SeriesId == seriesId && e.SceneAbsoluteEpisodeNumber == sceneAbsoluteEpisodeNumber).ToList();

        public async Task<List<Episode>> EpisodesBetweenDates(DateTime startDate, DateTime endDate, bool includeUnmonitored, bool includeSpecials)
        {
            var seriesById = includeUnmonitored ? null : (await _seriesRepository.All()).ToDictionary(s => s.Id);
        ...
        public async Task SetMonitoredFlat(Episode episode, bool monitored)
        {
            episode.Monitored = monitored;
            await SetFields(episode, e => e.Monitored);
            ModelUpdated(episode, true);
        }

        public async Task SetMonitoredBySeason(int seriesId, int seasonNumber, bool monitored)
        {
            foreach (var episode in (await All()).Where(e => e.SeriesId == seriesId && e.SeasonNumber == seasonNumber && e.Monitored != monitored).ToList())
            {
                episode.Monitored = monitored;
                await SetFields(episode, e => e.Monitored);
            }
        ...
        public async Task SetMonitored(IEnumerable<int> ids, bool monitored)
        {
            ...
            {
                await SetFields(new Episode { Id = id, Monitored = monitored }, e => e.Monitored);
            }
        }

        public async Task<List<int>> SetMonitored(int seriesId, MonitorTypes monitor, int firstSeason, int lastSeason)
        {
            if (monitor is MonitorTypes.MonitorSpecials or MonitorTypes.UnmonitorSpecials)
            {
                ...
                foreach (var episode in (await All()).Where(e => e.SeriesId == seriesId && e.SeasonNumber == 0 && e.Monitored != shouldMonitor).ToList())
                {
                    episode.Monitored = shouldMonitor;
                    await SetFields(episode, e => e.Monitored);
                }

                return await GetSeasonNumbersWithMonitoredEpisodes(seriesId);
            }

            if (monitor == MonitorTypes.None)
            {
                foreach (var episode in (await All()).Where(e => e.SeriesId == seriesId && e.Monitored).ToList())
                {
...
#pragma warning restore CS0612
            ...
            foreach (var episode in (await All()).Where(e => e.SeriesId == seriesId).ToList())
            {
        ...
        public async Task SetFileId(Episode episode, int fileId)
        {
            episode.EpisodeFileId = fileId;
            await SetFields(episode, e => e.EpisodeFileId);
            ModelUpdated(episode, true);
        }

        public async Task ClearFileId(Episode episode, bool unmonitor)
        {
            episode.EpisodeFileId = 0;
            episode.Monitored &= !unmonitor;
            await SetFields(episode, e => e.EpisodeFileId, e => e.Monitored);
            ModelUpdated(episode, true);
        }

        private async Task<List<int>> GetSeasonNumbersWithMonitoredEpisodes(int seriesId) =>
            (await All()).Where(e => e.SeriesId == seriesId && e.Monitored).Select(e => e.SeasonNumber).Distinct().ToList();
