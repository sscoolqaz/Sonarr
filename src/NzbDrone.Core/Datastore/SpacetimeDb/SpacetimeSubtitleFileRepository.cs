...
namespace NzbDrone.Core.Datastore.SpacetimeDb
{
    public class SpacetimeSubtitleFileRepository : SpacetimeExtraFileRepository<SubtitleFile, StdbSubtitleFile>, ISubtitleFileRepository
    {
        ...
        protected override IDisposable SubscribeOwnDeleteCommitted(Action<int> onCommitted, Action<Exception> onFailed)
        {
            ...
            Conn.Connection.Reducers.OnDeleteSubtitleFile += Handler;
            return new Unsubscriber(() => Conn.Connection.Reducers.OnDeleteSubtitleFile -= Handler);
        }

        protected override IDisposable SubscribeOwnInsertCommitted(Action onCommitted, Action<Exception> onFailed)
        {
            void Handler(ReducerEventContext ctx, int p1, int? p2, int? p3, string p4, SpacetimeDB.Timestamp p5, SpacetimeDB.Timestamp p6, string p7, string p8, int p9, string p10, string p11)
            {
                if (ctx.Event.CallerIdentity == Conn.Connection.Identity &&
                    ctx.Event.CallerConnectionId == Conn.Connection.ConnectionId &&
                    ctx.Event.Status is Status.Committed)
            ...
            Conn.Connection.Reducers.OnInsertSubtitleFile += Handler;
            return new Unsubscriber(() => Conn.Connection.Reducers.OnInsertSubtitleFile -= Handler);
        }
