...
namespace NzbDrone.Core.Datastore.SpacetimeDb
{
    public class SpacetimeQualityProfileRankRepository : SpacetimeBasicRepository<QualityProfileQualityRank, StdbQualityProfileQualityRank>, IQualityProfileRankRepository
    {
        ...
        protected override IDisposable SubscribeOwnDeleteCommitted(Action<int> onCommitted, Action<Exception> onFailed)
        {
            ...
            Conn.Connection.Reducers.OnDeleteQualityProfileQualityRank += Handler;
            return new Unsubscriber(() => Conn.Connection.Reducers.OnDeleteQualityProfileQualityRank -= Handler);
        }

        protected override IDisposable SubscribeOwnInsertCommitted(Action onCommitted, Action<Exception> onFailed)
        {
            void Handler(ReducerEventContext ctx, int p1, int p2, double p3)
            {
                if (ctx.Event.CallerIdentity == Conn.Connection.Identity &&
                    ctx.Event.CallerConnectionId == Conn.Connection.ConnectionId &&
                    ctx.Event.Status is Status.Committed)
            ...
            Conn.Connection.Reducers.OnInsertQualityProfileQualityRank += Handler;
            return new Unsubscriber(() => Conn.Connection.Reducers.OnInsertQualityProfileQualityRank -= Handler);
        }
